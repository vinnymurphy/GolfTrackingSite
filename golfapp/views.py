...
from .forms import (CoursePictureForm, GolferUserCreationForm, HoleForm,
                    ScoreForm, TeeColorForm, TeeForm)
from .models import Course, CoursePicture, Hole, Round, Score, Tee, TeeColor
...
class HomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_active:
            context["rounds"] = Round.objects.select_related().filter(
...
# The course page will display information about the holes and tees of
# a course.
class CourseDetail(DetailView):
    model = Course
    context_object_name = "course"

    # Get the holes associated with the course and the tees associated
    # with the holes
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["holes"] = Hole.objects.filter(
            course_id=self.object.pk
        ).order_by("number")
        context["tees"] = Tee.objects.select_related("hole")
        context["pictures"] = CoursePicture.objects.filter(
            course_id=self.object.pk
        ).order_by("created_on")
        return context
...
# In the future I want holes to be added autmatically. Hole number
# would increment.
...
# I do not believe this view is ever used...
#
#  When a user deletes a hole its tees are deleted, if a user needs to
#  change a tees data they would update an existing one.
#
# There is no real reason a user would want to manually delete a tee
...
# Planned to change the success url to the newly created round instead
# of the round list
...
class RoundDetail(DetailView):
    ...
    # Score calculation is done by comparing the total_par to the
    # rounds total_score
    #
    # Planned to change this to include womens par
    #
    # Planned to change this to automaticaly show only the score
    # difference relevent to the user's gender
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["holes"] = Hole.objects.filter(
            course_id=self.object.course.pk
        ).order_by("number")
        context["tees"] = Tee.objects.filter(color=self.object.tee_color)
        context["scores"] = Score.objects.filter(round_id=self.object.pk)
        context["total_score"] = Score.objects.filter(
            round_id=self.object.pk
        ).aggregate(total_score=Sum("strokes"))
        context["total_mens_par"] = Hole.objects.filter(
            course_id=self.object.course.pk
        ).aggregate(total_mens_par=Sum("mens_par"))
        context["total_womens_par"] = Hole.objects.filter(
            course_id=self.object.course.pk
        ).aggregate(total_womens_par=Sum("womens_par"))
        if context["total_score"].get("total_score") and (
            context["total_mens_par"].get("total_mens_par")
            or context["total_womens_par"].get("total_womens_par")
        ):
            if self.object.created_by.gender != "MALE":
                context["player_par"] = context["total_score"].get(
                    "total_score"
                ) - context["total_womens_par"].get("total_womens_par")
            else:
                context["player_par"] = context["total_score"].get(
                    "total_score"
                ) - context["total_mens_par"].get("total_mens_par")
